  ...
  return (
    <AdminLayout>
      <div className="space-y-6">
        <div>
          <h1 className="text-3xl font-bold text-slate-900">{t('admin.groups.title')}</h1>
          <p className="mt-2 text-slate-600">{t('admin.groups.subtitle')}</p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredGroups.map((group) => (
                <div key={group.id} className="bg-white border border-slate-200 rounded-lg p-6 hover:shadow-lg transition-shadow">
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-slate-500">{group.creator_name}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className="font-medium text-slate-900">{group.number_of_members}</span>
                    </div>
                    ...
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className="font-medium text-slate-900">${group.monthly_amount}</span>
                    </div>
          ...
          {filteredGroups.length === 0 && !loading && (
            <div className="text-center py-12">
              ...
              <h3 className="mt-2 text-sm font-medium text-slate-900">{t('admin.groups.noGroups')}</h3>
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white rounded-lg shadow-md p-6">
            <div className="text-sm font-medium text-slate-600">{t('admin.groups.totalGroups')}</div>
            <div className="mt-2 text-3xl font-bold text-slate-900">{groups.length}</div>
          </div>
          <div className="bg-white rounded-lg shadow-md p-6">
            <div className="text-sm font-medium text-slate-600">{t('admin.groups.activeGroups')}</div>
            <div className="mt-2 text-3xl font-bold text-green-600">
              {groups.filter(g => g.status === 'active').length}
            </div>
          </div>
          <div className="bg-white rounded-lg shadow-md p-6">
            <div className="text-sm font-medium text-slate-600">{t('admin.groups.totalMembers')}</div>
